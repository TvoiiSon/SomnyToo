...
use super::crypto_batch_processor::{
    CryptoBatchProcessor, CryptoBatch, CryptoOperation, BatchPriority, BatchResult, BatchCryptoConfig
};
...
impl BatchOrchestrator {
    ...
    pub async fn new(
        crypto_config: BatchCryptoConfig,
    ...
    async fn handle_batch_completed(&self, batch_id: u64, result: BatchResult) {
        ...
        // Обновляем статистику
        {
            ...
            if let Some(processing_time) = result.processing_time.checked_sub(Duration::from_nanos(0)) {
                let total = stats.total_batches as f64;
                let current_avg = stats.avg_batch_processing_time.as_nanos() as f64;
                let new_avg = (current_avg * (total - 1.0) + processing_time.as_nanos() as f64) / total;
                stats.avg_batch_processing_time = Duration::from_nanos(new_avg as u64);
            }
        }
    ...
    async fn update_queue_stats(&self, pending_batches: &Vec<(BatchPriority, CryptoBatch)>) {
        let mut stats = self.stats.lock().await;
        stats.queue_sizes = pending_batches.iter().map(|(_, b)| b.len()).collect();
    }
